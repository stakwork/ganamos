...
>(({ className, onChange, ...props }, ref) => {
  return (
    <RPNInput.default
      ref={ref}
      className={cn("flex bg-white border border-gray-300 rounded-lg", className)}
...
const InputComponent = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => (
  <Input className={cn("rounded-e-lg rounded-s-none pl-8 bg-transparent border-0", className)} {...props} ref={ref} />
))
...
const CountrySelectComponent = ({ disabled, value, onChange, options }: CountrySelectProps) => {
